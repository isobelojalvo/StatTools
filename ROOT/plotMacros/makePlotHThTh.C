...
makeLTauStack(TString name,TString file,TString dir,int s,TString labelX,TString units = "GeV",bool left=false,TString channel = "",TString json = "Golden",bool log = false,bool dndm=false,bool doRatio = false)
{
	...
	lumi_sqrtS = "13 TeV";
	if (json=="Golden") lumi_13TeV = channel+"    2016, 20 fb^{-1}";

	int iPeriod = 4;    // 1=7TeV, 2=8TeV, 3=7+8TeV, 7=7+8+13TeV 


	float xR=0.45; //legend parameters
...
makeDiTauStack(TString name,TString file,TString dir,int s,TString labelX,TString units = "GeV",bool left=false,TString channel = "",TString json = "Golden",bool log = false,bool dndm=false,bool doRatio = false)
{
	setTDRStyle();
	...
	lumi_7TeV  = "4.9 fb^{-1}";  // default is "5.1 fb^{-1}"
	lumi_sqrtS = "13 TeV";
	if (json=="Golden") lumi_13TeV = channel+"    20 fb^{-1}, 2016";
	...
	if(s==3) {
		TH1F * SM = (TH1F*)(f->Get(dir+"/ggH125"));
		TH1F * SMVBF = (TH1F*)(f->Get(dir+"/vbfH125"));
		TH1F *sm = SM;
		SMVBF->Scale(0.237); //cross section = 0.236 pb-1
		sm->Scale(2.768); // cross section = 2.768 pb-1
		sm->Add(SMVBF);
	...
	if(doRatio){
		...
		data2->GetXaxis()->SetLabelSize(0.11);
		data2->GetXaxis()->SetTitleSize(0.15);
		data2->GetXaxis()->SetTitleOffset(0.9);
		data2->Draw("P");
		line->Draw();
	...
	TLegend *l = new TLegend(xR,0.55,xR+0.3,0.9);
	l->AddEntry(data,"Observed","PEL");
